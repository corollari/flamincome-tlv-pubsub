...
export default async function computeAllAPYs() {
  const APYs = Object.entries(vaults).map(async ([token, { contract }]) => {
    ...
    const increaseSharePrice = new BN(await currentSharePrice)
      .mul(pow10(18))
      .div(new BN(await previousSharePrice));

    const diffBlockTime =
      Number((await currentBlock).timestamp) -
      Number((await previousBlock).timestamp);
    const fractionOfYear = new BN(365 * 24 * 60 * 60).div(
      new BN(diffBlockTime)
    );
    ...
    const percentageAPY =
      APY.div(pow10(yearUnit - precision)).toNumber() / 10 ** (precision - 2);
    return {
      token,
      percentageAPY,
    };
